...
const EmployeeMessages = () => {
  ...
  // Random contacts data (removed entries)
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Combine API users with random contacts, avoiding duplicates
      const allUsers = [...apiUsers, ...randomContacts.filter(rc => 
        !apiUsers.some(au => au.email === rc.email)
      )];
      setUsers(allUsers);
    } catch (error) {
      console.error('Failed to fetch users:', error);
      // If API fails, still show random contacts
      setUsers(randomContacts);
      toast.error('Failed to load some contacts, showing available ones');
    } finally {
      setLoading(false);
    }
  };
